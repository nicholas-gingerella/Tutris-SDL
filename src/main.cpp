...
void renderBox(SDL_Renderer *, int, int, int, int, Tutris::ColorRgb, Tutris::ColorRgb);

void renderBoxCenter(SDL_Renderer *, int, int, Tutris::ColorRgb, Tutris::ColorRgb);
...
const std::string resource_path_fonts = "../assets/fonts/";
const std::string resource_path_music = "../assets/music/";
const std::string resource_path_sounds = "../assets/sounds/";
...
Game *game_instance = new Game(SCREEN_WIDTH / 2 - ((Tutris::FIELD_WIDTH / 2) * Tutris::BLOCK_SIZE_PIXEL), 100,
                               Tutris::FIELD_WIDTH, Tutris::FIELD_HEIGHT);
const unsigned int DEFAULT_PIECE_FALL_SPEED = 5;
unsigned int piece_fall_counter = DEFAULT_PIECE_FALL_SPEED; // The lower the number, the faster the speed
unsigned int speed_counter = 0;
unsigned int elapsed_ms = 0;
unsigned int max_speed_lock = 4 * (60 * 1000); // used to max out speed if game is going on too long (4 mins)
unsigned int speed_up_interval = 20 * 1000; // speed up piece every 20s
...
int main(int argc, char **argv) {
    ...
    bgm_game = Mix_LoadMUS("../assets/music/bg_music.mp3");
    if (bgm_game == nullptr) {
        std::cout << Mix_GetError() << std::endl;
        SDL_Utils::cleanup(window);
        SDL_Quit();
    ...
    sfx_drop = Mix_LoadWAV("../assets/sounds/drop.wav");
    if (sfx_drop == nullptr) {
        std::cout << Mix_GetError() << std::endl;
        SDL_Utils::cleanup(window);
        SDL_Quit();
    }
    sfx_collapse = Mix_LoadWAV("../assets/sounds/collapse.wav");
    if (sfx_collapse == nullptr) {
        std::cout << Mix_GetError() << std::endl;
        SDL_Utils::cleanup(window);
        SDL_Quit();
    }
    sfx_row_clear = Mix_LoadWAV("../assets/sounds/row_clear.wav");
    ...
    sfx_gameover = Mix_LoadWAV("../assets/sounds/gameover.wav");
    ...
    sfx_pause_on = Mix_LoadWAV("../assets/sounds/pause_on.wav");
    ...
    sfx_pause_off = Mix_LoadWAV("../assets/sounds/pause_off.wav");
    ...
    sfx_victory = Mix_LoadWAV("../assets/sounds/victory.wav");
        ...
        switch (tutris_state) {
            case game_state::title: {
                if (!on_title_screen) {
                    ...
                    game_instance = new Game(SCREEN_WIDTH / 2 - ((Tutris::FIELD_WIDTH / 2) * Tutris::BLOCK_SIZE_PIXEL),
                                             100, Tutris::FIELD_WIDTH, Tutris::FIELD_HEIGHT);
                    elapsed_ms = 0;
                    score = 0;
                    updateGameScoreText(renderer, score);
            ...
            case game_state::playing: {
                ...
                } else if (!on_title_screen && (elapsed_ms >= Tutris::MAX_TIME_MS)) {
                ...
                if (!game_victory) {
                    if (game_running) {
                        ...
                        game_instance = new Game(
                                SCREEN_WIDTH / 2 - ((Tutris::FIELD_WIDTH / 2) * Tutris::BLOCK_SIZE_PIXEL), 100,
                                Tutris::FIELD_WIDTH, Tutris::FIELD_HEIGHT);
                        elapsed_ms = 0;
                        score = 0;
                        piece_fall_counter = DEFAULT_PIECE_FALL_SPEED;
                        updateGameScoreText(renderer, score);
                ...
                switch (event.key.keysym.sym) {
                    case SDLK_LEFT:
                        if (tutris_state == game_state::playing) {
                            game_instance->movePiece(Tutris::MoveDirection::left);
                        }
                        break;
                    case SDLK_RIGHT:
                        if (tutris_state == game_state::playing) {
                            game_instance->movePiece(Tutris::MoveDirection::right);
                        }
                        break;
                    case SDLK_DOWN:
                        if (tutris_state == game_state::playing) {
                            game_instance->movePiece(Tutris::MoveDirection::down);
                        }
                        break;
                    case SDLK_UP:
                        if (tutris_state == game_state::playing) {
...
void game_update(SDL_Renderer *rend) {
    ...
    if (elapsed_ms >= Tutris::MAX_TIME_MS) {
        game_victory = true;
        return;
    }
    ...
    if (!game_instance->isPieceActive()) {
        ...
        while (!clear_rows.empty()) {
            ...
            if (collapse) {
                score += (Tutris::SCORE_INCREMENT_COLLAPSE * neighbor_rows);
            ...
            // All cleared rows now handled and put in place.
        ...
        // Are we able to spawn a new piece to drop onto the board?
        if (!game_instance->addPiece(Tutris::PieceShape::random)) {
    ...
    // Is there an active piece and is it able to fall into free space?
...
void display_game_over_prompt(SDL_Renderer *rend) {
    ...
    int box_height = 200;
    renderBoxCenter(rend, box_width, box_height, Tutris::TUTRIS_COLOR_BLACK, Tutris::TUTRIS_COLOR_RED);

    int text_x_pos = SCREEN_WIDTH / 2 - (Tutris::FIELD_WIDTH * Tutris::BLOCK_SIZE_PIXEL) / 2 - 70;
...
void display_victory_prompt(SDL_Renderer *rend) {
    ...
    int box_width = Tutris::FIELD_WIDTH * Tutris::BLOCK_SIZE_PIXEL + 200;
    int box_height = 200;
    renderBoxCenter(rend, box_width, box_height, Tutris::TUTRIS_COLOR_BLACK, Tutris::TUTRIS_COLOR_GREEN);

    int text_x_pos = SCREEN_WIDTH / 2 - (Tutris::FIELD_WIDTH * Tutris::BLOCK_SIZE_PIXEL) / 2 - 60;
    int text_y_pos = SCREEN_HEIGHT / 2 - box_height / 2;
...
void display_title_prompt(SDL_Renderer *rend) {
    int box_width = Tutris::FIELD_WIDTH * Tutris::BLOCK_SIZE_PIXEL + 200;
    int box_height = 200;
    renderBoxCenter(rend, box_width, box_height, Tutris::TUTRIS_COLOR_BLACK, Tutris::TUTRIS_COLOR_WHITE);

    int text_x_pos = SCREEN_WIDTH / 2 - (Tutris::FIELD_WIDTH * Tutris::BLOCK_SIZE_PIXEL) / 2;
    int text_y_pos = SCREEN_HEIGHT / 2 - box_height / 2;
...
void display_Pause_prompt(SDL_Renderer *rend) {
    ...
    int text_x_pos = SCREEN_WIDTH / 2 - (Tutris::FIELD_WIDTH * Tutris::BLOCK_SIZE_PIXEL) / 2 - 100;
    int text_y_pos = SCREEN_HEIGHT / 2 - box_height / 2;
...
std::string get_countdown_timer(unsigned int elapsed_time_ms) {
    ...
    if ((elapsed_time_ms > 0) && (elapsed_time_ms <= Tutris::MAX_TIME_MS)) {
        max_ms_countdown = (Tutris::MAX_TIME_MS) - elapsed_time_ms;
    } else {
        max_ms_countdown = 0;
    }
...
void clearScreen(SDL_Renderer *r) {
    SDL_SetRenderDrawColor(r, Tutris::COLOR_BACKGROUND.r, Tutris::COLOR_BACKGROUND.g, Tutris::COLOR_BACKGROUND.b, 0xFF);
    SDL_RenderClear(r);
}

void renderScoreText(SDL_Renderer *r) {
    int score_x_pos = SCREEN_WIDTH / 2 + (game_instance->getNumCols() * Tutris::BLOCK_SIZE_PIXEL) / 2;
    score_x_pos += 10; // shift a few pixels away from game grid
    int score_y_pos = SCREEN_HEIGHT / 2 - (game_instance->getNumRows() * Tutris::BLOCK_SIZE_PIXEL) / 2;
    SDL_Utils::renderTexture(text_score, r, score_x_pos, score_y_pos);
}

void renderTimerText(SDL_Renderer *r) {
    int timer_x_pos = SCREEN_WIDTH / 2 + (game_instance->getNumCols() * Tutris::BLOCK_SIZE_PIXEL) / 2;
    timer_x_pos += 10; // shift a few pixels away from game grid
    int timer_y_pos = SCREEN_HEIGHT / 2 - (game_instance->getNumRows() * Tutris::BLOCK_SIZE_PIXEL) / 2;
